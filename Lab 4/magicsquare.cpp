...
MagicSquareGame::MagicSquareGame() : GameBase(3, 3, 0, 0) //Load pieces into available piece array, check for longest piece display
{
	...
	if (line == "MagicSquare") {
		...
		while (getline(ifs, line)) {
			...
			while (iss >> word) {
				int coordinate = 3;
				if (word.size() == coordinate && word[comma] == ',') {
					x = word[x_coord] - '0';
					y = word[y_coord] - '0';
				}
				else {
