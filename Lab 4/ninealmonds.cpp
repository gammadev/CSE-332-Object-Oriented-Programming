...
NineAlmonds::NineAlmonds() : GameBase(5, 5, 0, 0)
{
	ifstream ifs;
	...
	string line;
	getline(ifs, line);
	if (line == "NineAlmonds") { //Checks if first line is NineAlmonds
		getline(ifs, line);
		turns = stoi(line); //Gets turns played so far in the second line
		...
		while (getline(ifs, line)) {
			...
			while (iss >> word) {
				int coordinate = 3;
				if (word.size() == coordinate && word[comma] == ',') { //Get the coordinates of the game board from each line in the file
					x = word[x_coord] - '0';
					y = word[y_coord] - '0';
				}
				else {
					board[x][y].display = word; //Get the piece value on the line from each board coordinate
	...
	else { //Start game normally and fill up game board
