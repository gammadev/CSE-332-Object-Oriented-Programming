...
int usage(const string & program, const string & message)
{
	...
	if (message == "Please enter second command argument (with player names if a multiplayer game)") {
		return wrong_cmd_args;
	}
	else if (message == "Input file cannot be opened") {
		return file_not_open;
	}
	else if (message == "- Game not created") {
		cout << "Enter 'NineAlmonds', 'MagicSquare', or 'Reversi' after the program name" << endl;
		cout << "If entering 'Reversi', enter two player names after 'Reversi'" << endl;
		return game_not_given;
	}
	else {
		return success;
	}
